...
const addIntervalToDate = (date, value, unit) => {
  ...
  const operations = {const { logger } = require('@librechat/data-schemas');
const { ViolationTypes } = require('librechat-data-provider');
const { createAutoRefillTransaction, createModelAutoRefillTransaction } = require('./Transaction');
const { logViolation } = require('~/cache');
const { getMultiplier } = require('./tx');
...
const attemptAutoRefill = async ({
  ...
  if (
    ...
    try {
      ...
      logger.error(
        `[Balance.check] Failed to record ${logContext} auto-refill transaction`,
        error,
      );
    }
  }
 ...
 *
 * Prevents race conditions where multiple requests
 * read the same balance and overspend concurrently.
 *
 * MongoDB guarantees this update is atomic:
 *  - check tokenCredits >= tokenCost
 *  - decrement tokenCredits
 */
const spendTokens = async ({ user, endpoint, tokenCost }) => {
  const updatedRecord = await Balance.findOneAndUpdate(
    {
      user,
      'endpointLimits.endpoint': endpoint,
      'endpointLimits.enabled': true,
      'endpointLimits.tokenCredits': { $gte: tokenCost },
    },
    {
      $inc: { 'endpointLimits.$.tokenCredits': -tokenCost },
    },
    {
      new: true,
    },
...
const checkBalanceRecord = async function ({
  ...
  const endpointLimit =
    endpoint &&
    record.endpointLimits?.find((el) => el.endpoint === endpoint && el.enabled);

  // If no endpoint limit exists OR not enabled → ALLOW UNLIMITED
  if (!endpointLimit) {
    logger.debug('[Balance.check] No endpoint limit found - allowing unlimited', {
      user,
      endpoint,
    });
   ...
   *
   * We do atomic check+deduct in MongoDB.
   */
  let newRecord = await spendTokens({ user, endpoint, tokenCost });
  let canSpend = !!newRecord;

  /**
   * If atomic spend failed, try auto-refill (only if enabled).
   * Auto-refill respects interval; it will NOT trigger early.
   */
  if (!canSpend && endpointLimit.autoRefillEnabled && endpointLimit.refillAmount > 0) {
    const refillBalance = await attemptAutoRefill({
      user,
    ...
    /**
     * If refill happened, retry atomic spend.
     */
    if (refillBalance !== null) {
      newRecord = await spendTokens({ user, endpoint, tokenCost });
      canSpend = !!newRecord;
    }
  }

  /**
   * Update balance for return (real current value after atomic update).
   */
  balance = newRecord
    ? newRecord.endpointLimits.find((el) => el.endpoint === endpoint).tokenCredits
    : balance;
...
const checkBalance = async ({ req, res, txData }) => {
  const { canSpend, balance, tokenCost, balanceSource } =
...
let canSpend = !!newRecord;
 ...
 if(!canSpend){
   if(endpoint.autoRefillEnabled && endpoint.refillAmount > 0){
     const refillBalance = await attemptAutoRefill({
       user,
       lastRefill: endpointLimit.lastRefill,
       intervalValue: endpointLimit.refillIntervalValue,
