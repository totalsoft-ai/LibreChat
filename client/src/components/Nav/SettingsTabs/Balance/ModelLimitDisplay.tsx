...
import type { TModelLimit } from 'librechat-data-provider';
...
function ModelLimitDisplay({ limit }: ModelLimitDisplayProps) {
  ...
  return (
    <div className="rounded-lg border border-border-medium bg-surface-primary-alt p-4">
      ...
      <div className="mb-3 flex items-center justify-between">
        <h4 className="font-semibold text-text-primary">{limit.model}</h4>
        {!limit.enabled && (
          <span className="rounded-full bg-surface-tertiary px-2 py-1 text-xs text-text-secondary">
            {localize('com_nav_model_limit_disabled')}
          </span>
        )}
      </div>
      ...
      <div className="mb-2">
        <div className="flex items-baseline justify-between">
          <span className="text-sm text-text-secondary">
            {localize('com_nav_model_limit_credits')}
          </span>
      ...
      {limit.autoRefillEnabled && limit.refillAmount && limit.refillAmount > 0 && (
        <div className="mb-2 flex items-center gap-1 rounded bg-surface-secondary p-2 text-xs text-text-secondary">
          ...
          <span>
            {localize('com_nav_model_limit_auto_refill')}:{' '}
            <strong>{formatNumber(limit.refillAmount)}</strong>{' '}
            {localize('com_nav_model_limit_every')}{' '}
            <strong>
      ...
      <div className="text-xs text-text-secondary">
        {localize('com_nav_model_limit_last_used')}:{' '}
        {limit.lastUsed ? formatBalanceDateTime(limit.lastUsed) : localize('com_nav_model_limit_never')}
      </div>
    </div>
  );
}

export default React.memo(ModelLimitDisplay);
